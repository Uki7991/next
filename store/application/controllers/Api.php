...
class Api extends CI_Controller
{
    ...
    public function update_userdata()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->common_model->data_update("store_login", $insert_array, array("user_id" => $this->input->post("user_id")));

            $q = $this->db->query("Select * from `store_login` where(user_id='" . $this->input->post('user_id') . "' ) Limit 1");
            $row = $q->row();
            $data["responce"] = true;
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $q = $this->db->query("Select * from `store_login` where(user_email='" . $this->input->post('user_email') . "' ) and user_password='" . md5($this->input->post('password')) . "' Limit 1");
    ...
    public function sales_login()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //users.user_email='".$this->input->post('user_email')."' or
            $q = $this->db->query("Select * from `sales_rep_login` where(user_email='" . $this->input->post('user_email') . "' ) and user_password='" . md5($this->input->post('password')) . "' Limit 1");
...
//        $this->form_validation->set_rules('user_id', 'User ID', 'trim|required');
    ...
    function all_users()
    {
        ...
        $this->load->model("product_model");
        $data = $this->product_model->get_user();
    ...
    function all_products()
    {
        ...
        $this->load->model("product_model");
        $data = $this->product_model->all_products_list();
        ...
        if (_is_user_login($this)) {
            ...
            if (isset($_POST)) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $data = "Your request added successfully...";
                    echo json_encode($data);
                }
    ...
    public function assign_client_list()
    {
        $d = date('d/m/y');
        $id = $this->input->post("user_id");
        $q = $this->db->query("select * from `assign_client` where sale_user_id='" . $id . "' AND on_date ='" . $d . "'");
        if ($q->num_rows() > 0) {
            $data["responce"] = true;
            $data["msg"] = "client Available";
            $data['data'] = $q->result();
        } else {
            $data["responce"] = false;
            $data["msg"] = "Client Not Available";
            $data['data'] = $q->result();
        }
    ...
    public function assign_client_count()
    {
        ...
        if ($sale_user_id != 0) {
            $q = $this->db->query("SELECT * FROM `assign_client` where id='" . $sale_user_id . "'");
            $data["responce"] = true;
            $data["list"] = $q->num_row();
            echo json_encode($data);
        } else {
